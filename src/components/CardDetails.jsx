...
const PaymentForm = ({ subtotal }) => {
  ...
  const handleCheckout = async () => {
    if (validateForm()) {
        ...
        alert(`You have purchased successfully!  Total price: $${total})`);
    }
  };
    ...
    if (!cardNumberRegex.test(cardNumber)) {
        setCardNumberError('Card number must be at least 16 digits.');
        valid = false;
    } 
    else {
        setCardNumberError('');
    ...
    if (!expirationDateRegex.test(expirationDate)) {
        setExpirationDateError('Expiration date: month/year.');
        valid = false;
    } 
    else {
    ...
    if (cardType === '') {
        setCardTypeError('Select a card type.');
        valid = false;
    } 
    else {
         setCardTypeError('');
  ...
  const handleSeeAll = () => {
        setCardTypeMessage('Select one of the Card Types');
  };
